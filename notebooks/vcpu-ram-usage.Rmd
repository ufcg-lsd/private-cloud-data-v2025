# Plot of vcpu and RAM usage trough the time 
...
p_cpu <- df_long %>% 
  filter(metric == "vCPU usage [%]") %>% 
  make_panel(show_legend = TRUE,
             title = "(a) vCPU usage", lab ="vCPU usage [%]" )

p_ram <- df_long %>% 
  filter(metric == "RAM usage [%]") %>% 
  make_panel(show_legend = FALSE,
             title = "(b) RAM usage", lab = "RAM usage [%]")
